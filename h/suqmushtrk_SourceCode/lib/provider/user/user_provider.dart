...
import 'package:RoyalBoard_Common_sooq/ui/common/facebook_login_web_view.dart';
...
import 'package:RoyalBoard_Common_sooq/viewobject/holder/apple_login_holder.dart';
import 'package:RoyalBoard_Common_sooq/viewobject/holder/facebook_login_user_holder.dart';
import 'package:RoyalBoard_Common_sooq/viewobject/holder/fb_login_parameter_holder.dart';
import 'package:RoyalBoard_Common_sooq/viewobject/holder/google_login_parameter_holder.dart';
import 'package:RoyalBoard_Common_sooq/viewobject/holder/user_login_parameter_holder.dart';
import 'package:RoyalBoard_Common_sooq/viewobject/holder/user_register_parameter_holder.dart';
...
class UserProvider extends PsProvider {
  UserProvider(
    ...
    subscription = userListStream.stream.listen((PsResource<User> resource)async {
       ...
       _nftp =DateTime.parse("${await NTP.now()}");
  ...
  String selectedRadioBtnName = RoyalBoardConst.ORDER_TIME_ASAP;
  bool isClickDeliveryButton = true;
  bool isClickPickUpButton = false;
DateTime _nftp;
DateTime get timenow =>_nftp;
  ...
  LatLng getUserLatLng() {
    if (user == null ||
      ...
      return LatLng(double.parse(RoyalBoardConfig.lat), double.parse(RoyalBoardConfig.lng));
    }

    return LatLng(double.parse(user.data.userLat ?? RoyalBoardConfig.lat),
        double.parse(user.data.userLng ?? RoyalBoardConfig.lng));
  }
  ...
  bool hasLatLng() {
    ...
    if (_latlng == null ||
      ...
      if (_latlng.latitude.toString() == RoyalBoardConfig.lat &&
          _latlng.longitude.toString() == RoyalBoardConfig.lng) {
        return false;
      } else {
        return true;
      }
  ...
  Future<dynamic> postUafromrollet(
      Map<dynamic, dynamic> jsonMap,
      ) async {
    isLoading = true;
    ...
    _apiStatus = await _repo.postUafromrolletrepo(
        jsonMap, isConnectedToInternet, PsStatus.PROGRESS_LOADING);
  ...
  Future<void> handleFirebaseAuthError(BuildContext context, String email,
    ...
    if (provider.contains(RoyalBoardConst.emailAuthProvider) && !ignoreEmail) {
      showDialog<dynamic>(
          context: context,
    ...
    else if (provider.contains(RoyalBoardConst.googleAuthProvider)) {
      showDialog<dynamic>(
          context: context,
          builder: (BuildContext context) {
            return WarningDialog(
    ...
    else if (provider.contains(RoyalBoardConst.facebookAuthProvider)) {
      showDialog<dynamic>(
          context: context,
          builder: (BuildContext context) {
            return WarningDialog(
    ...
    else if (provider.contains(RoyalBoardConst.appleAuthProvider)) {
      showDialog<dynamic>(
          context: context,
          builder: (BuildContext context) {
            return WarningDialog(
  ...
  Future<FacebookLoginUserHolder> _getFirebaseUserWithFacebookId(
    ...
    // Get Keys
    const String yourClientId = RoyalBoardConfig.fbKey;
  ...
  Future<fb_auth.User> signInWithEmailAndPassword(
    ...
    try {
      ...
      if (user == null) {
        try {
          result = await _firebaseAuth.signInWithEmailAndPassword(
              email: RoyalBoardConst.defaultEmail, password: RoyalBoardConst.defaultPassword);
        } on PlatformException catch (e) {
          print(e);
          final fb_auth.User _user2 = await createUserWithEmailAndPassword(
              context, RoyalBoardConst.defaultEmail, RoyalBoardConst.defaultPassword,
              ignoreHandleFirebaseAuthError: true);
          if (_user2 != null) {
            user = _user2;
          }
  ...
  Future<fb_auth.User> createUserWithEmailAndPassword(
    ...
    try {
      ...
      print('provider : $provider');
      // Registered With Email
      if (provider.contains(RoyalBoardConst.emailAuthProvider)) {
        final fb_auth.User user =
            await signInWithEmailAndPassword(context, email, email);
        if (user == null) {
  ...
  Future<PsResource<User>> _submitSignUpWithEmailId(
    ...
    if (_apiStatus.data != null) {
      ...
      if (user.status == RoyalBoardConst.ONE) {
        replaceVerifyUserData('', '', '', '');
        replaceLoginUserId(_apiStatus.data.userId);
