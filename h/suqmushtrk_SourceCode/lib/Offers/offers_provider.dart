...
class OffersProvider extends PsProvider {
  OffersProvider({@required ProductRepository repo, int limit = 0})
      : super(repo, limit) {
    _repo = repo;
    ...
    subscription =
        productListStream.stream.listen((PsResource<List<Product>> resource) {
          ...
          _productList = Utils.setOffersProducts<Product>(resource);
          _FirmList = Utils.removeDuplicateObj<Product>(resource);
  ...
  PsResource<List<Product>> _FirmList =
  PsResource<List<Product>>(PsStatus.NOACTION, '', <Product>[]);

  PsResource<List<Product>> get productList => _productList;
  PsResource<List<Product>> get FirmList => _FirmList;
