...
import 'package:RoyalBoard_Common_sooq/ui/common/dialog/chat_noti_dialog.dart';
...
import 'package:RoyalBoard_Common_sooq/viewobject/holder/noti_register_holder.dart';
...
class Utils {
  ...
  static Future<File> getImageFileFromAssets(Asset asset, int imageSize) async {
    ...
    if (status) {
      ...
      final Directory _appTempDirFolder = Directory(
          '${_appTempDir.path}/${RoyalBoardConfig.tmpImageFolderName}');
  ...
  static dynamic getBannerAdUnitId() {
    if (Platform.isIOS) {
      return RoyalBoardConfig.iosAdMobUnitIdApiKey;
    } else {
      return RoyalBoardConfig.androidAdMobUnitIdApiKey;
    }
  }

  static dynamic getAdAppId() {
    if (Platform.isIOS) {
      return RoyalBoardConfig.iosAdMobAdsIdKey;
    } else {
      return RoyalBoardConfig.androidAdMobAdsIdKey;
    }
  }
  ...
  static String getDateFormat(String dateTime) {
    ...
    return DateFormat(RoyalBoardConfig.dateFormat).format(date);
  }
  ...
  static PsResource<List<T>> setOffersProducts<T>(
      PsResource<List<T>> resource) {
    final Map<String, String> _keyMap = HashMap<String, String>();
    final List<T> _tmpDataList = <T>[];
  ...
  static Future<void> saveDeviceToken(
    ...
    final NotiRegisterParameterHolder notiRegisterParameterHolder =
        NotiRegisterParameterHolder(
            platformName: RoyalBoardConst.PLATFORM,
            deviceId: fcmToken,
            loginUserId:
