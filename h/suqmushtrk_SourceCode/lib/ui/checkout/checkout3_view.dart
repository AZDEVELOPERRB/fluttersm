...
import 'package:RoyalBoard_Common_sooq/provider/token/token_provider.dart';
      ...
      if (userLoginProvider.user != null &&
        ...
        final PsResource<TransactionHeader> _apiStatus =
            await transactionSubmitProvider.postTransactionSubmit(
                ...
                '',
                RoyalBoardConst.ZERO,
                widget.isClickPickUpButton == true ? RoyalBoardConst.ONE : RoyalBoardConst.ZERO,
...
//here the server check function
  dynamic callCardNow(
      BasketProvider basketProvider,
      UserProvider userLoginProvider,
      TransactionHeaderProvider transactionSubmitProvider) async {
    if (await Utils.checkInternetConnectivity()) {
      if (userLoginProvider.user != null &&
        ...
        await PsProgressDialog.showDialog(context);
        print(
            'asfasfgsfa${basketProvider.checkoutCalculationHelper.subTotalPrice.toString()}');
        final PsValueHolder valueHolder =
            Provider.of<PsValueHolder>(context, listen: false);
        print('safkasjfhkjasfhska${basketProvider.checkoutCalculationHelper.subTotalPrice}');
        print('sdghkdsahjk${ basketProvider.checkoutCalculationHelper.subTotalPrice
            .toString()}');
        if(double.parse('${basketProvider.checkoutCalculationHelper.subTotalPrice}')!=0){
          final PsResource<TransactionHeader> _apiStatus =
          await transactionSubmitProvider.postTransactionSubmit(
              userLoginProvider.user.data,
              widget.basketList,
  ...
  dynamic payRazorNow(
    ...
    if (psValueHolder.standardShippingEnable == RoyalBoardConst.ONE) {
    ...
    } else if (psValueHolder.zoneShippingEnable == RoyalBoardConst.ONE) {
      basketProvider.checkoutCalculationHelper.calculate(
          basketList: widget.basketList,
          couponDiscountString: couponDiscountProvider.couponDiscount,
          psValueHolder: psValueHolder,
    ...
    final Map<String, Object> options = <String, Object>{
      ...
      'currency': RoyalBoardConfig.isRazorSupportMultiCurrency
          ? _shopInfoProvider.shopInfo.data.currencyShortForm
          : RoyalBoardConfig.defaultRazorCurrency,
      'description': '',
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Container(
                width: 50,
                height: 50,
                child: Icon(Foundation.paypal, color: RoyalBoardColors.white)),
            Container(
              margin: const EdgeInsets.only(
                left: PsDimens.space16,
                right: PsDimens.space16,
              ...
              child: Text(Utils.getString(context, 'checkout3__paypal'),
                  style: Theme.of(context)
                      .textTheme
                      .subtitle1
                      .copyWith(height: 1.3, color: RoyalBoardColors.white)),
            ),
          ],
        ),
      ),
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Container(
                width: 50,
                height: 50,
                child: Icon(Icons.payment, color: RoyalBoardColors.white)),
            Container(
              margin: const EdgeInsets.only(
                left: PsDimens.space16,
                right: PsDimens.space16,
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Container(
                width: 50,
                height: 50,
                child: Icon(Ionicons.md_cash, color: RoyalBoardColors.white)),
            Container(
              margin: const EdgeInsets.only(
                  left: PsDimens.space16, right: PsDimens.space16),
